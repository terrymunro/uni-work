...
void* MemManage::Alloc(int size)
{
   ...
   // Find unallocated space large enough to fit
	...
	if (mbPtr == NULL)
	{
		// Case: No unused blocks large enough found, add new to the end.
		MemoryBlock mb, lastInserted = memoryBlocks.back();

		// Special Case: Not enough space left at the end (due to fragmentation)
		if (lastInserted.startPtr + size > memoryBlocks.front().startPtr + maxSpace)
			return NULL;
